...
static ZEND_FUNCTION(definition_exists);
...
ZEND_BEGIN_ARG_INFO_EX(arginfo_definition_exists, 0, 0, 1)
	ZEND_ARG_INFO(0, definitionname)
	ZEND_ARG_INFO(0, autoload)
ZEND_END_ARG_INFO()
...
static const zend_function_entry builtin_functions[] = { /* {{{ */
	...
	ZEND_FE(definition_exists,	arginfo_definition_exists)
...
/* {{{ proto bool definition_exists(string definitionname [, bool autoload])
 Checks if the definition exists */
ZEND_FUNCTION(definition_exists)
{
	char *definition_name, *lc_name;
	zend_class_entry **ce;
	int definition_name_len;
	int found;
	zend_bool autoload = 1;
	ALLOCA_FLAG(use_heap)

	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "s|b", &definition_name, &definition_name_len, &autoload) == FAILURE) {
		return;
	}
	...
	if (!autoload) {
		...
		lc_name = do_alloca(definition_name_len + 1, use_heap);
		zend_str_tolower_copy(lc_name, definition_name, definition_name_len);

		/* Ignore leading "\" */
		name = lc_name;
		len = definition_name_len;
		if (lc_name[0] == '\\') {
			name = &lc_name[1];
			len--;
		}
		...
		found = zend_hash_find(EG(class_table), name, len+1, (void **) &ce);
		free_alloca(lc_name, use_heap);
		RETURN_BOOL(found == SUCCESS && (((*ce)->ce_flags & ZEND_ACC_DEFINITION) >= ZEND_ACC_DEFINITION));
	}

	if (zend_lookup_class(definition_name, definition_name_len, &ce TSRMLS_CC) == SUCCESS) {
		RETURN_BOOL(((*ce)->ce_flags & ZEND_ACC_DEFINITION) >= ZEND_ACC_DEFINITION);
	} else {
		RETURN_FALSE;
	}
}
